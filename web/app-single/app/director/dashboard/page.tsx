...
export default function DirectorDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setKpis({
        totalStudents: (students.results || students).length || 0, totalCourses: (courses.results || []).length || 0,
        totalAircraft: (aircraft.results || []).length, totalFlights: fltList.length,
        completedFlights: fltList.filter((f: any) => f.status === "completed").length, totalFlightHours: Math.round(totalHours),
        revenueCollected: Math.round(paid), revenueOutstanding: Math.round(outstanding),
