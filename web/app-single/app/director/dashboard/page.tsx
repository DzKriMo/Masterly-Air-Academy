...
export default function DirectorDashboard() {
  ...
  return (<div className="min-h-screen bg-navy-900">
    <nav className="sticky top-0 bg-navy-800/95 backdrop-blur border-b border-navy-700 z-50"><div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between"><div className="flex items-center gap-3"><Image src="/logo.png" alt="MAA" width={110} height={110}/><div><h1 className="text-lg font-bold text-white">{t('director.dashboard', 'Director Dashboard')}</h1><p className="text-xs text-gold-500">{t('director.executiveOverview', 'Executive Overview')}</p></div></div><div className="flex items-center gap-3"><ExportButton exports={[{label:"Students (Excel)",url:"/export/students/",filename:"students.xlsx",type:"excel"},{label:"Invoices (Excel)",url:"/export/invoices/",filename:"invoices.xlsx",type:"excel"},{label:"Flights (Excel)",url:"/export/flights/",filename:"flights.xlsx",type:"excel"}]}/><button onClick={async()=>{await logout();router.push("/login")}} className="px-4 py-2 text-sm text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/10">{t('common.signOut', 'Logout')}</button></div></div></nav>
