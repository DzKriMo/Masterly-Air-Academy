...
export default function DirectorDashboard() {
  ...
  const { data, isLoading: loading } = useQuery({
    ...
    queryFn: () => Promise.all(["/api/students/","/api/invoices/","/api/courses/","/api/aircraft/","/api/flight-lessons/","/api/audits/"].map(fetchJSON)),
  ...
  const charts={flights:Object.entries(sc).map(([n,v])=>({name:n,value:v})),revenue:[{name:"Collected",value:Math.round(paid)},{name:"Outstanding",value:Math.round(out)}],fleet:aList.map((a:any)=>({name:a.registration,hours:parseFloat(a.airframe_hours)||0})),invoices:Object.entries(ic).map(([n,v])=>({name:n,value:v}))};
  ...
  return (<div className="min-h-screen bg-navy-900">
    ...
    <main className="max-w-7xl mx-auto px-6 py-8">{loading?<p className="text-gray-500">Loading KPIs...</p>:<>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <ChartCard title="Flights by Status"><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={charts.flights} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({name,value}:any)=>`${name}: ${value}`}>{charts.flights.map((_:any,i:number)=><Cell key={i} fill={DCOLORS[i%DCOLORS.length]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <ChartCard title="Invoice Status"><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={charts.invoices} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({name,value}:any)=>`${name}: ${value}`}>{charts.invoices.map((_:any,i:number)=><Cell key={i} fill={DCOLORS[i%DCOLORS.length]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
      </div>
