        ...
        {tab === "compose" && (
          <form onSubmit={handleSend} className="bg-navy-800 border border-navy-700 rounded-xl p-6 mb-8">
            <div className="space-y-4">
              <div><label className="block text-sm text-gray-400 mb-1">To</label><select value={form.receiver} onChange={e => setForm({...form, receiver: e.target.value})} required className="w-full px-3 py-2.5 bg-navy-900 border border-navy-600 rounded-lg text-white text-sm"><option value="">Select student...</option>{users.map((u: any) => <option key={u.id} value={u.user_id || u.id}>{u.full_name} ({u.student_number})</option>)}</select></div>
