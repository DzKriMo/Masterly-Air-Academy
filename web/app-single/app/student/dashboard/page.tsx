...
export default function StudentDashboard() {
  ...
  return (<div className="min-h-screen bg-navy-900">
    <nav className="sticky top-0 bg-navy-800/95 backdrop-blur border-b border-navy-700 z-50"><div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between"><div className="flex items-center gap-3"><Image src="/mast.svg" alt="MAA" width={110} height={110} className="rounded-lg"/><div><h1 className="text-lg font-bold text-white">Masterly Air Academy</h1><p className="text-xs text-gold-500">Student Portal</p></div></div><div className="flex items-center gap-4"><span className="text-sm text-gray-400 hidden sm:inline">{user?.name||user?.email}</span><button onClick={async()=>{await logout();router.push("/student/login")}} className="px-4 py-2 text-sm text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/10">Logout</button></div></div></nav>
    <main className="max-w-6xl mx-auto px-6 py-8"><h2 className="text-2xl font-bold text-white mb-2">Welcome back, {user?.name?.split(" ")[0]||"Student"}</h2><p className="text-gray-400 mb-8">Your training overview</p>
      {dataLoading?<p className="text-gray-500">Loading stats...</p>:<>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4"><QT href="/student/exams" label="Exams"/><QT href="/student/flights" label="Flight Log"/><QT href="/student/courses" label="Courses"/><QT href="/student/schedule" label="Schedule"/><QT href="/student/messages" label="Messages"/></div>
      </>}</main></div>);
}
...
function QT({href,label}:{href:string;label:string}){return <a href={href} className="flex items-center justify-center p-5 bg-navy-800 border border-navy-700 rounded-xl hover:border-gold-500 text-gray-300 hover:text-gold-500 transition-all text-sm font-medium min-h-[90px]">{label}</a>}
