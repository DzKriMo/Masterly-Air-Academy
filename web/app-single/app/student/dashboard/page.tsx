...
export default function StudentDashboard() {
  ...
  const [stats, setStats] = useState({ courses: 0, completed: 0, attendance: 0, flightHours: 0, examAvg: 0, flights: 0, upcoming: 0 });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setStats({
        courses: prog.total_courses || 0,
        completed: prog.completed_courses || 0,
        attendance: prog.attendance_rate || 0,
        flightHours: log.total_flight_hours || 0,
        examAvg: scores.length > 0 ? Math.round(scores.reduce((a: number,b: number) => a+b, 0) / scores.length) : 0,
        flights: log.total_lessons || 0,
        upcoming: 0,
      });
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      <nav className="sticky top-0 bg-navy-800/95 backdrop-blur border-b border-navy-700 z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-bold text-white">Masterly Air Academy</h1><p className="text-xs text-gold-500">Student Portal</p></div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <p className="text-gray-400 mb-8">Here is your training overview</p>
        ...
        {dataLoading ? <p className="text-gray-500">Loading stats...</p> : (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
              ...
              <StatCard label="Completed" value={stats.completed} sub={`${stats.attendance}% attendance`} />
              <StatCard label="Flight Hours" value={`${stats.flightHours}h`} sub={`${stats.flights} lessons`} />
              <StatCard label="Exam Average" value={stats.examAvg > 0 ? `${stats.examAvg}%` : "—"} sub="Best: 100%" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <QuickTile href="/student/exams" label="Results" />
            </div>
          </>
        )}
      </main>
