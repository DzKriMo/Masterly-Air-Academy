        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-navy-800 rounded-xl border border-navy-700 p-6">
                <h3 className="text-sm font-semibold text-gray-400 mb-4 uppercase tracking-wider">{t('student.groundScores', 'Ground Scores')}</h3>
                {groundList.length > 0 ? (
                  <div className="space-y-4">
                    <div className="text-center">
                      <p className="text-4xl font-bold text-white">{groundAvg !== null ? `${groundAvg}%` : "-"}</p>
                      <p className="text-xs text-gray-400 mt-1">{t('student.groundAverage', 'Average Score')}</p>
                    </div>
                    <div className="bg-navy-900 rounded-lg p-4">
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-gray-300">{t('student.validated', 'Validated')}</span>
                        <span className="text-gray-500">{groundValidated}/{groundTotal}</span>
                      </div>
                      <div className="w-full bg-navy-800 rounded-full h-2">
                        <div className="bg-green-500 h-2 rounded-full transition-all" style={{ width: `${groundTotal > 0 ? (groundValidated / groundTotal) * 100 : 0}%` }} />
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-center h-[220px]">
                    <p className="text-gray-500 text-sm">{t('student.noGroundData', 'No ground evaluations yet.')}</p>
                  </div>
                )}
              </div>
            </div>
