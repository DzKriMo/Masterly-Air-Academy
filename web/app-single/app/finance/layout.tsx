...
export default function FinanceLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && !["finance_responsible", "accounting_agent", "system_admin"].includes(user.role)) { router.push("/login"); return; }
    ...
    <div className="min-h-screen bg-navy-900 flex">
      ...
      `}>
        <div className="lg:hidden flex items-center justify-between p-4 border-b border-navy-700">
          <p className="text-white font-bold text-sm">{t("layout.financePortal")}</p>
        ...
        <div className="p-4 border-b border-navy-700 hidden lg:block">
          ...
          <p className="text-white font-bold text-center mt-2 text-sm">{t("layout.financePortal")}</p>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-navy-800/95 backdrop-blur border-b border-navy-700 h-14 flex items-center gap-3" style={{ padding: "env(safe-area-inset-top) 16px 0" }}>
        ...
        <span className="text-white font-semibold text-sm truncate flex-1">{user?.name || user?.email || "Finance"}</span>
      </div>
