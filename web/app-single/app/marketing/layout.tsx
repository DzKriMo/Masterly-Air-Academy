...
import { LayoutDashboard, LayoutList, Images, Bell, MessageSquare, Menu, X, Globe } from "lucide-react";
...
export default function MarketingLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && !["social_manager", "system_admin"].includes(user.role)) { router.push("/login"); return; }
    ...
    <div className="min-h-screen bg-navy-900 flex">
      ...
      `}>
        <div className="lg:hidden flex items-center justify-between p-4 border-b border-navy-700">
          <p className="text-white font-bold text-sm">{t("layout.marketingPortal")}</p>
        ...
        <div className="p-4 border-b border-navy-700 hidden lg:block">
          ...
          <p className="text-white font-bold text-center mt-2 text-sm">{t("layout.marketingPortal")}</p>
        ...
        <nav className="p-2 flex-1 overflow-y-auto">
          {NAV.map(item => (
            <a
              key={item.href}
              href={(item as any).external ? item.href : item.href}
              target={(item as any).external ? "_blank" : undefined}
              onClick={(item as any).external ? undefined : closeSidebar}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm mb-1 transition-colors ${pathname===item.href?"bg-gold-500/20 text-gold-500 font-medium":"text-gray-400 hover:text-white hover:bg-navy-700"}`}
            >
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-navy-800/95 backdrop-blur border-b border-navy-700 h-14 flex items-center gap-3" style={{ padding: "env(safe-area-inset-top) 16px 0" }}>
        ...
        <span className="text-white font-semibold text-sm truncate flex-1">{user?.name || user?.email || "Marketing"}</span>
      </div>
