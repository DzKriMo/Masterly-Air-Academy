...
export default function AdminPracticalEvaluationsPage() {
  ...
  const { data: records, isLoading, error, refetch } = useQuery<PE[]>({
    queryKey: ["admin-pe"],
    queryFn: async () => { const d = await api.get<any>("/practical-evaluations/"); return (d as any)?.results || (d as any) || []; },
  ...
  const columns: Column<PE>[] = useMemo(() => [
    ...
    { key: "actions", header: "", render: (a) => (
      <div className="flex gap-1 justify-end" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => { setEditItem(a); setEditForm({ student: a.student, instructor: a.instructor, lesson_type: a.lesson_type || "", date: a.date, result: a.result || "", grade: a.grade || "", observations: a.observations || "", strengths: a.strengths || "", improvements: a.improvements || "", recommendations: a.recommendations || "", decision: a.decision || "" }); setEditError(""); }} className="px-2 py-1 text-xs text-gold-500 hover:bg-gold-500/10 rounded transition-colors">Edit</button>
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      <PageHeader title="Practical Evaluations" backHref="/admin/dashboard" backLabel={t("common.back", "Back")} actions={
