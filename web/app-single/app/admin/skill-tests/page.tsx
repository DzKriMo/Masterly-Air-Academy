...
export default function AdminSkillTestsPage() {
  ...
  const { data: records, isLoading, error, refetch } = useQuery<ST[]>({
    queryKey: ["admin-st"],
    queryFn: async () => { const d = await api.get<any>("/skill-tests/"); return unwrapResults(d); },
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      <PageHeader title={t("admin.skillTests", "Skill Tests")} backHref="/admin/dashboard" backLabel={t("common.back", "Back")} actions={
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <ModalForm open={!!selected} onClose={() => setSelected(null)} title="Skill Test Details" footer={<button onClick={() => setSelected(null)} className="px-4 py-2 text-sm text-gray-400 border border-navy-700 rounded-lg hover:text-white">{t('common.close')}</button>}>
