...
export default function AdminCompetenciesPage() {
  ...
  const { data: records, isLoading, error, refetch } = useQuery<Competency[]>({
    ...
    queryFn: async () => { const d = await api.get<any>("/competencies/"); return (d as any)?.results || (d as any) || []; },
  ...
  const columns: Column<Competency>[] = useMemo(() => [
    ...
    { key: "actions", header: "", render: (a) => (
      <div className="flex gap-1 justify-end" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => { setEditItem(a); setEditForm({ student: a.student, program: a.program, competency: a.competency, status: a.status, achieved_at: a.achieved_at || "", notes: a.notes || "" }); setEditError(""); }} className="px-2 py-1 text-xs text-gold-500 hover:bg-gold-500/10 rounded transition-colors">Edit</button>
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      <PageHeader title="Competencies" backHref="/admin/dashboard" backLabel={t("common.back", "Back")} actions={
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <ModalForm open={!!selected} onClose={() => setSelected(null)} title="Competency Details" footer={<button onClick={() => setSelected(null)} className="px-4 py-2 text-sm text-gray-400 border border-navy-700 rounded-lg hover:text-white">Close</button>}>
