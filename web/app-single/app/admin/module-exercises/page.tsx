...
export default function AdminModuleExercisesPage() {
  ...
  const { data: records, isLoading, error, refetch } = useQuery<ModuleExercise[]>({
    ...
    queryFn: async () => { const d = await api.get<any>("/module-exercises/"); return unwrapResults(d); },
  ...
  const columns: Column<ModuleExercise>[] = useMemo(() => [
    ...
    { key: "actions", header: "", render: (a) => (
      <div className="flex gap-1 justify-end" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => { setEditItem(a); setEditForm({ module: a.module, title: a.title, instructions: a.instructions || "", due_date: a.due_date || "" }); setEditError(""); }} className="px-2 py-1 text-xs text-gold-500 hover:bg-gold-500/10 rounded transition-colors">{t('common.edit')}</button>
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      <PageHeader title={t("admin.moduleExercises", "Module Exercises")} backHref="/admin/dashboard" backLabel={t("common.back", "Back")} actions={
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <ModalForm open={!!selected} onClose={() => setSelected(null)} title="Exercise Details" footer={<button onClick={() => setSelected(null)} className="px-4 py-2 text-sm text-gray-400 border border-navy-700 rounded-lg hover:text-white">{t('common.close')}</button>}>
