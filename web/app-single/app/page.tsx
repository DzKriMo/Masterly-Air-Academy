...
const dLbls: Record<string, Record<string, string>> = {
  en: { ppl: "6-8 months", cpl: "12-18 months", ir: "3-4 months", mep: "1-2 months", mcc: "2-3 weeks" },
  fr: { ppl: "6-8 mois", cpl: "12-18 mois", ir: "3-4 mois", mep: "1-2 mois", mcc: "2-3 semaines" },
  ar: { ppl: "٦-٨ أشهر", cpl: "١٢-١٨ شهراً", ir: "٣-٤ أشهر", mep: "١-٢ شهر", mcc: "٢-٣ أسابيع" },
};
const pLbls: Record<string, Record<string, string>> = {
  en: { ppl: "Class 2 Medical Certificate", cpl: "PPL + Class 1 Medical Certificate", ir: "PPL + 50 hours cross-country", mep: "CPL or in progress", mcc: "CPL + IR" },
  fr: { ppl: "Certificat Medical Classe 2", cpl: "PPL + Certificat Medical Classe 1", ir: "PPL + 50h navigation", mep: "CPL ou en cours", mcc: "CPL + IR" },
  ar: { ppl: "شهادة طبية فئة ٢", cpl: "PPL + شهادة طبية فئة ١", ir: "PPL + ٥٠ ساعة", mep: "CPL أو قيد التقدم", mcc: "CPL + IR" },
};

export default function LandingPage() {
  const { t, locale } = useTranslation();
  const dLbl = dLbls[locale] || dLbls.en;
  const pLbl = pLbls[locale] || pLbls.en;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {programKeys.map((code) => (
              <div key={code} className="group bg-navy-900 border border-navy-700 rounded-xl p-6 hover:border-gold-500/40 transition-all">
                ...
                <h3 className="text-lg font-bold text-white mb-2 group-hover:text-gold-500 transition-colors">{programTitles[code]}</h3>
                ...
                <div className="border-t border-navy-700 pt-4 space-y-1.5">
                  <div className="flex justify-between text-xs"><span className="text-gray-500">{durLabel}</span><span className="text-gray-300">{dLbl[code.toLowerCase()]}</span></div>
                  <div className="flex justify-between text-xs"><span className="text-gray-500">{preLabel}</span><span className="text-gray-300">{pLbl[code.toLowerCase()]}</span></div></div></div>))}
          </div></div>
      </section>
